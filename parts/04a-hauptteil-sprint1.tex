...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{assets/robo-labeled.png}
\caption{Roboter}
...
Es kam bereits der Motortreiber zum Einsatz, der auch im späteren Endprodukt verbaut werden soll. Der zugrunde liegende Fahralgorithmus basiert darauf, die Werte beider Encoder zu mitteln, um die gefahrene Strecke zu ermitteln (siehe Gleichung~\ref{eq:gefahren}).
...
Nachdem die Ticks der Encoder pro Radumdrehung herausgefunden wurden, konnte dies in der MCCAR Library in der Quad Klasse konfiguriert werden. Danach wurde mithilfe eines Lineals geprueft, ob der Roboter sich wirklich die Distanz vorbewegt, wie der Encoder das sagt. Da dies erfolgreich war, konnte die Drive LIbr

ALINA NOTES:

to check that encoder works, check ticks, eine radumdrehung, schauen bei beiden raedern, daraus average und ticks per revolution in encoder
Mit Lineal mm zahl checken von lib: encoder geprueft, sagt das richtige

jetzt regler anschauen, da encoder zuveralessig speed liefern muss, jetz kann regler fuer regeln benutzt werden um motoren gleichschnell drehen

Using quad=encoder and dfive (regler) lib from mcfun that already work together out of the box
drive already asks quad for speed und anhan von speed von beiden motoren wird berechnet, dass greade gahrt: allese gegeben
damit motoren gleich schnell drehen
