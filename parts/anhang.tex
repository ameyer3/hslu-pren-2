...
\begin{itemize}
    ...
    \item Precision-Confidence: Zeigt die Balance zwischen Precision (Anteil von `True Positives'; wenn Model sagt, es gibt einen Knoten, wie akkurat ist diese Deutung?) und Confidence. Je tiefer die Confidence, desto besser, da dadurch der höchste Präzisionswert bereits bei einer tieferen Confidence erreicht wird.
    ...
    \item Lernverlauf: Zeigt inwiefern der Verlust (Unterschied zwischen Vorhergesagtem und Realität) sinkt bei den Trainingsdaten und den Validationsdaten. Beginnt der Verlust bei der Validation zu steigen, deutet dies auf Overfitting hin\footnote{https://developers.google.com/machine-learning/crash-course/overfitting/overfitting}. Sollte exponentiell sinken. Zeigt, wie das Model an Wissen gewinnt. Sollte exponentiell steigen.
\end{itemize}
...
Die ersten Tests mit dem Gyroskop wurden mit einem Bread Board durchgeführt und wurde mit einem Tiny K22 verbunden. Auf diese Weise kann das Gyroskop isoliert von den anderen Komponenten getestet werden. Das Setup ist ersichtlich auf Abbildung \ref{img:gyro-tests-1}.
