...
Das Gyroskop wurde fest an den Roboter angebracht. Um zu identifizieren ob die Werte brauchbar sind, hat man das Gyroskop alle paar Milisekunden ausgelesen und die im Speicher vorhandenen Werte wie auf Abb. \ref{fig:Werte Gyroskop} analysiert. Man musste dann feststellen das schon kleine Vibrationen die Werte zu sehr beeinflussen und so ist es nicht möglich eine exakte Drehung zu berechnen. 


\begin{figure}[h]
\centering
\includegraphics[width=3cm,height=4cm]{assets/ET/Gyroskop/Gyroskop_Werte.jpg}
\caption{Werte der Y-Achse}
\label{fig:Werte Gyroskop}
\end{figure}
...
Damit der Nachkorrekturwert, der gefahren werden muss, damit das Hindernis am selben Ort abgestellt werden kann, ermittelt werden kann, werden 10 Testdurchläufe durchgeführt. Dabei wird wiederholt der Roboter das Hindernis aufheben, sich 180\textdegree \ drehen und dann dort wo er ist, das Hindernis wieder hinstellten. Dies jeweiligen Abstellorte wurden mit Postit Zetteln markiert. Die Mitte von jedem Postit korrespondeirt mit der Mitte der Barriere. Auch der ursprüngliche Ort der Barriere wird markiert. Die Distanzen zwischen dem Abstellort und dem ursprüngliche Ort der Barriere wird gemessen. 
