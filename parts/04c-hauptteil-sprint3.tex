...
Dafür wurde auf dem Raspberry Pi ein Prototypen-HAT montiert, welcher die benötigten GPIO-Pins entsprechend dem Schema \ref{fig:raspiheader} auf Taster, Display und Buzzer verbindet.

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth, trim=7cm 7cm 7cm 7cm, clip]{assets/ET/PCB/raspiheader.pdf}
    \caption{Raspberry Pi HAT Schema}
    \label{fig:raspiheader}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{assets/ET/PCB/raspiheader.pdf}
    \caption{Raspberry Pi HAT Schema}
    \label{fig:raspiheader}
\end{figure}
...
Die Zieleingabe wird dazu verwendet, um vor dem Start den geforderten Zielknoten auszuwählen. Dazu wurde ein GPIO Pin definiert, welcher bei einer Steigenden Flanke, Also bei einem Knopfdruck zwischen den Zielknoten A --> B --> C hin und herschaltet. So wird nur ein Taster benötigt, um zwischen den drei Zielen zu wechseln.


\textbf{Startknopf}

Der Zweite Taster ist der Startknopf. Dieser wird nach eingabe des Zielknoten gedrückt, nach einer kurzen Wartezeit wird dann der Start initialisiert und der Roboter fängt an den Graphen richtung Zielknoten zu traversieren.
...
Zusätzlich wird der Status des Roboters dargestellt und die Laufzeit. Dadurch muss nicht separat eine Stoppuhr verwendet werden. Die Laufzeit wird automatisch beim erreichen des Zielknoten gestoppt.
